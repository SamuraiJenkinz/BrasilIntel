...
<task type="auto">
  ...
  <action>
    **app/config.py** — Remove ALL Apify and legacy scraping configuration:
    ...
    3. Remove the `is_apify_configured()` method (lines 173-175):
       ```python
    ...
    4. Remove the `get_source_timeout()` method (lines 177-191):
       ```python
    ...
    **app/routers/runs.py** — Remove legacy Apify pipeline functions:

    1. Delete the entire `_execute_single_insurer_run()` function (lines 364-470). This is Phase 2 legacy code that creates `ScraperService()` — dead code since Factiva replaced Apify.

    2. Delete the entire `_execute_category_run()` function (lines 473-564). This is Phase 3 legacy code that creates `ScraperService()` — dead code since Factiva replaced Apify.

    3. Update the module docstring (lines 1-8) to remove references to "Single insurer processing (Phase 2 compatibility)" and "Full category processing (Phase 3 scale)". The docstring should reflect Factiva-only pipeline:
       ```python
       """
    ...
    DO NOT remove any imports at the top of runs.py — they are all used by the active pipeline.
  </action>
  <verify>
  ...
  <done>config.py has no Apify fields or legacy scraping settings. runs.py has no legacy Apify pipeline functions. Only Factiva pipeline path remains.</done>
</task>
...
6. **Full remnant scan**: `grep -ri "apify\|ScraperService\|BatchProcessor\|SourceRegistry" app/ --include="*.py"` — no matches
</verification>

<success_criteria>
- app/config.py has no Apify/scraping fields (apify_token, batch_size, scrape_timeout, source_timeout removed)
